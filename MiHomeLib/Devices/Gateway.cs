...
public class Gateway(string sid, IMessageTransport transport) : MiHomeDevice(sid, TypeKey)
{
    public enum Sound
    {
        PoliceСar1 = 0,
    ...
    public override string ToString() => $"Rgb: {Rgb}, Illumination: {Illumination}, ProtoVersion: {ProtoVersion}";
    ...
    public void DisableLight() => _transport.SendWriteCommand(Sid, Type, new GatewayLightCommand(0));

    public void SoundsOff() => _transport.SendWriteCommand(Sid, Type, new PlaySoundCommand(1000, 0));
    ...
    public void StopPlayMusic() => _transport.SendWriteCommand(Sid, Type, new PlaySoundCommand(1000, 0));
}