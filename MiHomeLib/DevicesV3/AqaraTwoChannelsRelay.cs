...
public class AqaraTwoChannelsRelay : ZigBeeManageableDevice
{
    ...
    public event Action OnChannel1StateChange;    
    public event Action OnChannel2StateChange;        
    ...
    public AqaraTwoChannelsRelay(string did, IMqttTransport mqttTransport, ILoggerFactory loggerFactory) : base(did, mqttTransport, loggerFactory)
    {
        ResNamesToActions.Add(CHANNEL1_RES_NAME, x =>
        {
            ...
            OnChannel1StateChange?.Invoke();
        });

        ResNamesToActions.Add(CHANNEL2_RES_NAME, x =>
        {
            var state = x.GetInt32();
        ...
        ResNamesToActions.Add(ENERGY_RES_NAME, x =>
        {
            (var oldValue, Energy) = (Energy, (float)x.GetDouble());
            OnEnergyChange?.Invoke(oldValue);
        });

        ResNamesToActions.Add(LOAD_CURRENT_RES_NAME, x =>
        {
            (var oldValue, Current) = (Current, (float)x.GetDouble());
            OnCurrentChange?.Invoke(oldValue);
        });
    }
    ...
    /// <summary>
    /// Restore or not previous state after electricity was disrapted and then appeared again
    /// </summary>    
